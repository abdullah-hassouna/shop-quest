...
import {
  getAllProducts,
  getAllProductsPages,
} from "@/actions/admin/products/get-all-products";
...
export function ProductsDataTable({
  ...
  const [products, setProducts] =
    useState<GetProductDataResponse[]>(initialData);
  const [page, setPage] = useState<number>(1);
  const [maxPagesCount, setMaxPagesCount] =
    useState<number>(initialMaxPagesCount);
  const [pageTakeNum, setPagesTakeNum] = useState<number>(
    tableConfig.defaultPageSize
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }: { row: any }) => (
        <Badge
          ...
          style={{ background: row.getValue("category").color }}
        >
          {row.getValue("category").name}
          {row.getValue("category").icon ? (
            <img src={row.getValue("category").icon} className="w-3 h-3" />
          ) : (
            <></>
          )}
        </Badge>
    ...
    },
    {
      ...
      cell: ({ row }: { row: any }) =>
        row.getValue("tags").length ? row.getValue("tags")[0].name : "No Tags",
    },
    {
      title: "Added At",
    ...
    },
    {
      ...
      accessorKey: "actions",
      header: () => <>Actions</>,
      cell: ({ row }: { row: any }) => <ProductDataOptions row={row} />,
    },
  ];
  ...
  const recallAllData = useCallback(
    async (
      pageNum: number,
      take: number = 5,
      searchQuery: string = "",
      orderByObj: OrderBy | null
    ) => {
      setLoading(true);
      setError(null);
  ...
  const callPagesNumber = useCallback(
    async (take: number = 5, searchQuery: string = "") => {
      try {
        const { data, error, success } = await getAllProductsPages(
          take,
          searchQuery
        );
        if (success && data) {
        ...
        <Table className="w-full">
          ...
          <TableBody className="divide-y divide-gray-200">
            {products.map((user, rowIndex) => (
              <tr
                onClick={(e) => toggleToSelected(e, user.id as string)}
                key={user.id || rowIndex}
                className={cn("hover:bg-gray-50", {
                  "bg-gray-100": selectedList.find((OldId) => OldId == user.id),
                })}
              >
                {columns.map((column, colIndex) => (
                  <td
                    key={colIndex}
                    className="cursor-pointer px-4 py-2 text-sm text-gray-900"
                  >
                    {column.cell
                      ? column.cell({
                          row: {
                            getValue: (key: string) =>
                              user[key as keyof GetProductDataResponse],
                          },
                        })
                      : user[
                          column.accessorKey as keyof GetProductDataResponse
                        ]}
                  </td>
                ))}
              </tr>
